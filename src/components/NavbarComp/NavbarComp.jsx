...
const CardNav = ({
  ...
  const getCardPath = (label) => {
    if (label.toLowerCase() === 'home') return '/';
    return `/${label.toLowerCase()}`;
  };

  const handleCardClick = (e, item, onNavigate) => {
    if (e.target.closest('a')) return;
    const path = getCardPath(item.label);
    if (onNavigate) {
      onNavigate(path);
    } else {
        ...
        >
          {(items || []).slice(0, 3).map((item, idx) => (
            <div
              ...
              onClick={(e) => handleCardClick(e, item, onNavigate)}
            >
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  <a
                    key={`${lnk.label}-${i}`}
                    className="nav-card-link inline-flex items-center gap-[6px] no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] relative z-10"
                    href={lnk.href}
...
// Wrapper component with default configuration
const NavbarComp = () => {
  const contextValue = useContext(NavigationContext);
  const [, setPendingPath] = contextValue || [null, () => {}];
