...
const ActionCenter = () => {
  ...
  const fetchLatestAction = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('http://localhost:5000/api/action-center/latest');
      if (res.ok) {
  ...
  const handleApprove = async () => {
    ...
    setIsLoading(true);
    try {
       const res = await fetch('http://localhost:5000/api/action-center/approve', {
           method: 'POST',
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
        const res = await fetch('http://localhost:5000/api/action-center/send', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                response_id: actionData.id, 
